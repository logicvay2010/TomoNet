...
def tom_deconv_tomo(vol_file, out_file, angpix, voltage, cs, defocus, snrfalloff, deconvstrength, highpassnyquist, phaseflipped, phaseshift, ncpu=8, \
    ...
    # print("a:" , np.min(deconv), np.max(deconv))
    # print(std_deconv)
    deconv /= std_deconv
    # print("b:" , np.min(deconv), np.max(deconv))
    # print(std_vol)
    deconv *= std_vol
    # print("c:" , np.min(deconv), np.max(deconv))
    # print(ave_vol)
    deconv += ave_vol
        ...
        for i in range(self._N[0]):
            for j in range(self._N[1]):
                for k in range(self._N[2]):
                    ...
                    file_name = self.isonet_folder + '/deconv_temp/'+root_name+'_{}_{}_{}.mrc'.format(i,j,k)
...
def deconv_one(tomo, out_tomo, isonet_folder="IsoNet", voltage=300.0, \
    ...
    if os.path.isdir('{}/deconv_temp'.format(isonet_folder)):
    ...
    if chunk_size is None or chunk_size == 0:
        ...
        with Pool(ncpu) as p:
            partial_func = partial(tom_deconv_tomo, out_file=None, angpix=pixel_size, voltage=voltage, cs=cs, defocus=defocus, snrfalloff=snrfalloff,
                    deconvstrength=deconvstrength, highpassnyquist=highpassnyquist, phaseflipped=False, phaseshift=0, ncpu=1, global_min = global_min, global_max = global_max) 
    ...
    t2 = time.time()
    if not logger == None:
        log(logger, 'time consumed: {:10.4f} s'.format(t2-t1))
...
if __name__ == "__main__":
    ...
    if not len(argv) == 2:
        ...
        try:
            deconv_params = SearchParam(argv[1])
        except Exception as err:
            log(logger, err, "error")
            log(logger, "There is formating issue with the input JSON file {}".format(argv[1]), "error")
            sys.exit()

    deconv_one(deconv_params.tomo_name, deconv_params.deconv_tomo_name, deconv_params.isonet_folder, \
                    deconv_params.voltage, deconv_params.cs, deconv_params.defocus, \
                    deconv_params.pixel_size, deconv_params.snrfalloff, \
                    deconv_params.deconvstrength, deconv_params.highpassnyquist, \
                    deconv_params.chunk_size, deconv_params.overlap_rate, deconv_params.ncpu,\
                    logger=logger)